			...
			public void afterTextChanged(Editable s) {
				if(s.length()!=0){
					...
					if(petrolPrice != null){
						...
						discount = qty;
						binding.tvPetrolQty.setText("Petrol/Diesel (in liter) approx : "+ qty +" L");
						binding.tvDiscountReceived.setText("Discount (approx) : " + qty + " INR");
					}else {
						binding.tvPetrolQty.setText("Petrol/Diesel (in liter) approx : "+ "0" +" L");
			...
			if(amount.isEmpty() || amount.length()<=1){
				...
				if(!hasChosenImage){
					...
					if(filePath == null || filePath.isEmpty()){
						...
						mergeUser.put(UID, false);
						FirebaseFirestore.getInstance().collection("bpclref").document("ufcoll")
								.set(mergeUser, SetOptions.merge()).addOnSuccessListener(bbVoid ->
